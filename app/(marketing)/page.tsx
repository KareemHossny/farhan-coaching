...
export default function MarketingPage() {
  ...
  return (
    <main className="marketing-shell min-h-screen overflow-x-clip bg-[var(--bg-ink)] text-[var(--text-primary)]">
      ...
      <section id="pricing" className="pricing-section section-padding"><div className="container"><div className="pricing-intro mx-auto max-w-2xl text-center"><span className="eyebrow">{pricing.eyebrow}</span><h2 className="section-title mt-4">{pricing.title}</h2><p className="mt-4 text-[var(--text-muted)]">{pricing.subtitle}</p></div><div className="mt-12 grid gap-5 lg:grid-cols-2 lg:mx-auto lg:max-w-4xl">{pricing.plans.map((plan, index) => <Card key={plan.name} className={`pricing-card ${plan.highlighted ? "pricing-card-featured" : ""}`}><CardHeader><div className="pricing-card-topline"><span>0{index + 1} / متابعة</span>{plan.highlighted && <span className="pricing-badge">الأكثر طلبًا</span>}</div><div className="flex items-start justify-between gap-3"><CardTitle className="mt-5 text-2xl font-extrabold text-[var(--text-primary)]">{plan.name}</CardTitle></div><p className="mt-6 text-4xl font-black text-[var(--accent)]">{plan.price}</p><p className="text-sm text-[var(--text-muted)]">{plan.duration}</p><div className="pricing-checkin mt-6"><span>نظام الـ Check-In</span><strong>{plan.checkIn}</strong><p>{plan.checkInDetails}</p></div></CardHeader><CardContent><ul className="space-y-4 border-t border-[var(--border-hairline)] pt-6 text-sm text-[var(--text-secondary)]">{plan.features.map((feature) => <li key={feature} className="flex gap-3"><Check size={17} className="shrink-0 text-[var(--accent)]" />{feature}</li>)}</ul><a href={whatsappUrl(pricing.whatsappNumber, `${pricing.whatsappMessage} ${plan.name}`)} target="_blank" rel="noreferrer" className="mt-8 block"><Button className={plan.highlighted ? "cta-button w-full" : "secondary-button w-full"}>{plan.ctaText}<WhatsAppIcon size={17} /></Button></a></CardContent></Card>)}</div><div className="pricing-difference mt-8"><p className="eyebrow">الفرق بين الباقات</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><div><span>{pricing.plans[0].name}</span><strong>{pricing.plans[0].checkIn}</strong></div><div><span>{pricing.plans[1].name}</span><strong>{pricing.plans[1].checkIn}</strong></div></div></div><p className="mt-8 text-center text-sm text-[var(--text-muted)]">{pricing.note}</p></div></section>
