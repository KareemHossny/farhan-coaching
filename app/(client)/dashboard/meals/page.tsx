...
export default async function MealsPage() {
  ...
  return <div className="mx-auto max-w-4xl"><header className="flex items-start gap-4"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[rgb(226,253,75,0.1)] text-[var(--accent)]"><Utensils size={22} /></span><div><p className="eyebrow">خطة التغذية</p><h2 className="mt-2 text-3xl font-black">وجباتك اليومية</h2><p className="mt-2 leading-7 text-[var(--text-muted)]">خطة ثابتة لكل الأيام: الوجبة 1، الوجبة 2، والبدائل المتاحة بالجرام.</p></div></header><div className="mt-6"><PlanView diet={diet} today={new Date().getDay()} section="diet" /></div></div>;
}
