...
import { ExercisePerformanceSection, type ExercisePerformanceLog } from "@/components/client/ExercisePerformanceSection";
...
export default async function ClientDashboardPage() {
  ...
  const [{ data: safeItems, error: planError }, { data: performance }] = await Promise.all([
    ...
    supabase.from("exercise_performance_logs").select("plan_item_id, date, max_weight_kg, max_reps").eq("client_id", user.id).order("date", { ascending: false }),
  ]);
