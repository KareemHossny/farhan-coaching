    ...
    <>
      <div className="transformations-carousel relative mt-10 px-8 sm:px-12">
        <Swiper
          ...
          navigation={{ nextEl: ".transformations-next", prevEl: ".transformations-prev" }}
          autoplay={{ delay: 2600, disableOnInteraction: false, pauseOnMouseEnter: false, stopOnLastSlide: false, waitForTransition: true }}
          speed={750}
          centeredSlides
          loop
        ...
        >
          {repeatedItems.map((item, index) => (
            <SwiperSlide key={`${item.src}-${index}`} className="transformation-slide">
              <button type="button" className="transformation-card group" onClick={() => setSelected(item)} aria-label={`عرض قصة ${item.alt}`}>
        ...
        <button type="button" className="transformations-prev carousel-arrow right-0" aria-label="النتيجة السابقة"><ChevronRight size={22} /></button>
        <button type="button" className="transformations-next carousel-arrow left-0" aria-label="النتيجة التالية"><ChevronLeft size={22} /></button>
      </div>
